...
{
    public class MeshToSdfStatic : IDisposable
    {
        public struct InputArgs
        {
            public Vector3Int Resolution;
            public FloodMode FloodMode;
            public FloodFillQuality Quality;
        ...
        private void SetFromArgs(InputArgs args)
        {
            resolution = args.Resolution;
            floodMode = args.FloodMode;
            floodFillQuality = args.Quality;
            offset = args.Offset;
        ...
        private Bounds GetBounds()
        {
            ...
            if (m_MeshFilter)
            {
                if (m_MeshFilter.TryGetComponent(out MeshRenderer renderer))
...
If you need signed distance or just need a limited shell around your surface, use linear flood fill, but many iterations are expensive.")]
        ...
        [SerializeField]
        [Tooltip(
            "Offset the surface by the offset amount by adding it to distances in the final generation step. Applicable only in signed distance mode.")]
        float m_Offset = 0;

        public FloodMode floodMode
        {
            get { return m_FloodMode; }
            set { m_FloodMode = value; }
        }

        public FloodFillQuality floodFillQuality
        {
            get { return m_FloodFillQuality; }
            set { m_FloodFillQuality = value; }
        }
        ...
        static class Uniforms
        {
            ...
            internal static int _VertexBufferPosAttributeOffset =
                Shader.PropertyToID("_VertexBufferPosAttributeOffset");
        ...
        void RenderSDF(CommandBuffer cmd)
        {
            ...
            kernel = m_DistanceMode == DistanceMode.Signed && m_FloodMode == FloodMode.Linear
                ? m_SplatTriangleDistancesSignedKernel
                : m_SplatTriangleDistancesUnsignedKernel;
            ...
            cmd.SetComputeIntParam(m_Compute, Uniforms._VertexBufferPosAttributeOffset,
                m_VertexBufferPosAttributeOffset);
            ...
            if (m_FloodMode == FloodMode.Linear)
            {
                ...
                kernel = m_FloodFillQuality == FloodFillQuality.Normal
                    ? m_LinearFloodStepKernel
                    : m_LinearFloodStepUltraQualityKernel;
                for (int i = 0; i < m_FloodFillIterations; i++)
                {
                    cmd.SetComputeBufferParam(m_Compute, kernel, Uniforms._SDFBuffer,
                        i % 2 == 0 ? bufferPing : bufferPong);
                    cmd.SetComputeBufferParam(m_Compute, kernel, Uniforms._SDFBufferRW,
                        i % 2 == 0 ? bufferPong : bufferPing);
                    cmd.DispatchCompute(m_Compute, kernel, dispatchSizeX, dispatchSizeY, 1);
                }
                ...
                for (int i = 0; i < jumpFloodStepCount; i++)
                {
                    ...
                    if (m_FloodFillQuality == FloodFillQuality.Normal)
                    {
                        ...
                        for (int j = 0; j < 3; j++)
                        {
                            ...
                            cmd.SetComputeBufferParam(m_Compute, kernel, Uniforms._JumpBuffer,
                                bufferFlip ? m_JumpBuffer : m_JumpBufferBis);
                            cmd.SetComputeBufferParam(m_Compute, kernel, Uniforms._JumpBufferRW,
                                bufferFlip ? m_JumpBufferBis : m_JumpBuffer);
                            cmd.DispatchCompute(m_Compute, kernel, dispatchSizeX, dispatchSizeY, 1);
            ...
            cmd.SetComputeTextureParam(m_Compute, kernel, Uniforms._SDF, _sdfTexture);
            cmd.SetComputeFloatParam(m_Compute, Uniforms._Offset,
                m_DistanceMode == DistanceMode.Signed && m_FloodMode != FloodMode.Jump ? m_Offset : 0);
            cmd.DispatchCompute(m_Compute, kernel, dispatchSizeX, dispatchSizeY, 1);
        ...
        bool LoadMeshToComputeBuffers()
        {
            ...
            {
                Debug.LogError("MeshToSDF needs a mesh with triangle topology.");
                return false;
            }
            ...
            if (stream < 0)
            {
                Debug.LogError("MeshToSDF: no vertex positions in mesh '" + mesh.name + "', aborting.");
                return false;
            }
            ...
            m_VertexBuffer = m_SkinnedMeshRenderer != null
                ? m_SkinnedMeshRenderer.GetVertexBuffer()
                : mesh.GetVertexBuffer(stream);
        ...
        Matrix4x4 GetMeshToSDFMatrix(Transform transform)
        {
            ...
            if (m_SkinnedMeshRenderer != null)
            {
                if (m_SkinnedMeshRenderer.rootBone != null)
                    meshToWorld = m_SkinnedMeshRenderer.rootBone.localToWorldMatrix *
                                  Matrix4x4.Scale(m_SkinnedMeshRenderer.rootBone.lossyScale).inverse;
                else
