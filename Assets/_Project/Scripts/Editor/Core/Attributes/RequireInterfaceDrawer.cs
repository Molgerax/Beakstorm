...
namespace Beakstorm.Core.Attributes
{
    ...
    public class RequireInterfaceDrawer : PropertyDrawer
    {
        ...
        void DrawInterfaceObjectField(Rect position, SerializedProperty property, GUIContent label, Type interfaceType)
        {
            ...
            //newReference = EditorGUI.ObjectField(position, label, oldReference, baseType, true);

            SearchContext searchContext = SearchService.CreateContext($"t:{interfaceType.Name}", SearchFlags.OpenPicker);
            newReference = ObjectField.DoObjectField(position, oldReference, baseType, label, searchContext);
