Shader "DynaMak/Raymarching/DensityVolumeRaymarch"
{
    ...
    #pragma target 3.5
	#pragma shader_feature_fragment _ ATTENUATION
    #pragma shader_feature_fragment _ AMBIENT_LIGHT
    //#pragma skip_variants POINT_COOKIE DIRECTIONAL_COOKIE SPOT SPOT_COOKIE SHADOWS_SCREEN LIGHTMAP_SHADOW_MIXING
    //#pragma skip_variants SHADOWS_SCREEN LIGHTMAP_SHADOW_MIXING
    ...
    #define DEPTH_TEXTURE _LastCameraDepthTexture
    ...
    fixed4 fragBase (v2f i) : SV_Target
    {
        ...
        float nonLinearDepth = SAMPLE_DEPTH_TEXTURE(DEPTH_TEXTURE, screenCoords) * dot(rayDir, i.cameraViewNormal);
        ...
        for (int step = 0; step < _NumSteps; step++) {
            ...
            if (skipStep == 1)
            {
                ...
                rayPos = rayOrigin + rayDir * (dstToBox + dstTravelled);
